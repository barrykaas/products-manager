...
import { getWords } from 'src/utils/strings';
import { isInteger, isNumber, roundDigits } from 'src/utils/numbers';
import { formatEuro } from 'src/utils/monetary';
import { BrandLabel } from 'src/features/brands';
import { productQuantityDescription } from 'src/utils/productQuantity';
import { usePaginatedProducts } from 'src/features/products';
...
export default function QuickAddBox({ handleListItem, marketId }) {
    ...
    //     { enabled: !!search });
    const productsQuery = usePaginatedProducts({ search, market: marketId });
    const products = productsQuery.data ? productsQuery.data.pages[0].results : [];
    // const products = [];
